...
public class NewsService {   
    ...
    //正则需要过滤得标签
    private Pattern[] tagsDongtai = {FORM,TABLE,A,BACKGROUND_COLOR,IFRAME,IMG};
    ...
    public News getNews(Integer itemId) {
        ...
        News news = null;
        try {
            news = cacheHandler.getCacheValue(sb.toString(),News.class);
   ...
   public Information getNews(Integer newsId, Integer cityId,Integer pageNo,String cityName) {
       ...
       //news 如需要可加缓存,缓存住news实体,分页无需查第一个接口
       try {
    ...
    */
    ...
    public void reloadNewsData() {
        ...
        try{
             ...
             */
            boolean getLock = cacheClient.add(CACHE_LOCK_NEWS_KEY, 30, "ok");
            if(getLock) {
