...
public class BuildingDongtaiListController {
    ...
    // 动态新闻截断字数pad
    private static final int TRUNCATION_NUM_PAD = 100;
    ...
    public String getDongtaiList(Device device, @Param("cityName") @DefValue("bj") String cityName,
        ...
        if (build != null) {
            ...
            recentNewsList = decorate(recentNewsList, whichDevice);
        ...
        return whichDevice + "/dynamicList";
    }
    ...
    public String getDongTaiListAjax(Device device, Invocation inv,
    ...
    private List decorate(List<RecentNews> recentNewsList, String whichDevice) throws ParseException {
        ...
        if (recentNewsList != null && recentNewsList.size() > 0) {
            for (RecentNews recentNews : recentNewsList) {
                ...
                if (info != null) {
                    ...
                    if (recentNews.getInfocontent().length() > TRUNCATION_NUM && whichDevice.equals("phone")) {
                        recentNews.setInfocontent(recentNews.getInfocontent().substring(0, TRUNCATION_NUM)
                                + "...");
                    } else if (recentNews.getInfocontent().length() > TRUNCATION_NUM_PAD
                            && whichDevice.equals("pad")) {
                        recentNews.setInfocontent(recentNews.getInfocontent()
                                .substring(0, TRUNCATION_NUM_PAD) + "...");
