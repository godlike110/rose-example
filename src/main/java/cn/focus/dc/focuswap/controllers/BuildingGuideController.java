...
public class BuildingGuideController {
     ...
     */
    @Get("/")
    public String getHouseGuideList(Invocation inv,Device device,@Param("cityName") @DefValue("bj") String cityName) {
...
//        inv.addModel("returnFlag", returnFlag);
        return DeviceUtils.device(inv, device) + "/baodianList";
    }
     ...
     */
    @Get("{itemId:[0-9]+}/")
    public String getHouseGuide(Invocation inv,Device device,
        ...
        if (null != housingGuide) {
            ...
            if("share".equals(from)){
                ...
                returnTo =  DeviceUtils.device(inv, device) + "/baodianShare";
            }else{                
                returnTo =  DeviceUtils.device(inv, device) + "/baodian";            
            }
            inv.addModel("_city", city);
        } else {
            return "e:404";
