				...
				<article class="article_search">
					<div class="search_wrap">
						<form name="searchForm" method="post" action="${ctx }/${_city.cityPinyinAbbr }/search/list/" class="search_form clearfix">
							<div class="form_item">
								<input type="text" name="group_name" placeholder="输入楼盘名称/地点" autocomplete="off" value="${group_name }" />
							</div>
					...
					<div class="results">在${_city.cityName }共找到${total}个<h1 class="search_text">${group_name }</h1>楼盘</div>
				</article>
					...
					<div class="scroller">
						<article>
							...
							<c:forEach var="bl" items="${buildingList}" varStatus="status">
								<li role="touch_bg">
									<a href="${bl.buildingUrl }" class="clearfix">
						...
						<section class="crumb">
							<div class="inner">
								<a href="/${_city.cityPinyinAbbr }/">新房</a>&gt;<span>${group_name }楼盘</span>
							</div>
						</section>
					</div>
				</div>
